...
- `Pick`\<[`JWTDecodeParams`](jwt.mdx#jwtdecodeparams), `"salt"` \| `"secret"`\>
...
- `Record`\<`string`, `unknown`\>.[`DefaultJWT`](jwt.mdx#defaultjwt)
...
`Promise`\<`R` extends `true` ? `string` : [`JWT`](jwt.mdx#jwt) \| `null`\>
