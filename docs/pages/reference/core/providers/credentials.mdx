...
| :------ | :------ |
| `CredentialsInputs` extends `Record`\<`string`, [`CredentialInput`](credentials.mdx#credentialinput)\> | `Record`\<`string`, [`CredentialInput`](credentials.mdx#credentialinput)\> |
...
| :------ | :------ | :------ |
| `credentials` | `Partial`\<`Record`\<keyof `CredentialsInputs`, `unknown`\>\> | The available keys are determined by [CredentialInput](credentials.mdx#credentialinput).<br /><br />**Note**<br />The existence/correctness of a field cannot be guaranteed at compile time,<br />so you should always validate the input before using it.<br /><br />You can add basic validation depending on your use case,<br />or you can use a popular library like [Zod](https://zod.dev) for example. |
| `request` | [`Request`]( https://developer.mozilla.org/en-US/docs/Web/API/Request ) | The original request. |
...
| Parameter | Type |
| :------ | :------ |
| `config` | `Partial`\<[`CredentialsConfig`](credentials.mdx#credentialsconfigcredentialsinputs)\<`CredentialsInputs`\>\> |
